/*
 * qt_archive_file_browser.cpp (updated with lazy loading, node types, icons, and nested path status)
 * Qt 5.12 single-file demo: Virtual archive-aware file browser
 *
 * Features:
 * - Browse filesystem and custom virtual archives (*.vfsarc)
 * - Nested archives with lazy loading
 * - Node types: File, Folder, ArchiveFolder
 * - Metadata auto-handling via hidden .manifest.json
 * - Toolbar, breadcrumb navigation, and docked metadata viewer
 * - Double-click preview (text, images, external apps)
 * - Context menu for Add Folder / Remove / Metadata
 * - Drag & drop into archive
 * - Status bar shows nested archive path (folder1.vfsarc > folder2.vfsarc)
 * - Icons differentiate files, folders, and nested archives
 *
 * Build:
...
public:
    ...
    bool extractEntryToTemp(const QString &entry, QString &outPath) override {
        ...
        QProcess p;
        p.start("unzip", {m_archive, entry, "-d", persistentTmp});
        p.waitForFinished(-1);
        if (p.exitCode() == 0) {
    ...
    bool extractAll(const QString &destDir) override {
        QProcess p;
        p.start("unzip", {m_archive, "-d", destDir});
        p.waitForFinished(-1);
        return p.exitCode() == 0;
    }
...
public:
    MainWindow() {
        ...
        connect(archiveView, &QTreeView::customContextMenuRequested,
                this, &MainWindow::onArchiveContextMenu);
    ...
    void onArchiveContextMenu(const QPoint &pos) {
        ...
        if (selected == addFolder) {
            ...
            if (ok && !name.isEmpty()) {
                ...
                archiveModel->layoutChanged(); // refresh
                // TODO: update backend with placeholder entry if desired
            }
        } else if (selected == removeItem) {
            it->parent->children.removeOne(it);
            delete it;
            archiveModel->layoutChanged(); // refresh
        ...
        if (it->type == ArchiveItem::NodeType::ArchiveFolder) {
            ...
            if (backend->extractEntryToTemp(entry, tmp)) {
                ...
                if (nested->openArchive(tmp)) {
                    archiveModel->clear();
                    archiveModel->populateFromList(nested->listEntries());
                    auto meta = loadMetadata(nested);
...
private:
    void previewFile(const QString &path) {
        QFileInfo fi(path);
        QMimeDatabase db;
        QMimeType mt = db.mimeTypeForFile(path); // <-- create MIME type object
        ...
        // fallback: open with default system application
